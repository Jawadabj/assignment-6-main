            ...
            if (this.cameraControls.hasMoved) 
            {
                ...
                this.update(deltaTime);{
                    ...
                    if (this.drawState === DrawState.NO_DRAWING) {
                        ...
                        if (this.cameraControls.hasMoved) {
                            // Step 1: Get the camera's XZ position
                            const cameraXZPosition = new gfx.Vector3(this.camera.position.x, 0, this.camera.position.z);
                
                            // Step 2: Retrieve the triangle directly underneath the camera
                            const triangle = this.ground.getTriangleAtPosition(cameraXZPosition.x, cameraXZPosition.z);
                            const [v1, v2, v3] = triangle;
                
                            // Step 3: Create a ray pointing directly downward from the camera's XZ position
                            const downwardRay = new gfx.Ray3();
                            downwardRay.set(cameraXZPosition, new gfx.Vector3(0, -1, 0)); // Downward direction
                
                            // Step 4: Perform a ray-triangle intersection test
                            const groundIntersection = downwardRay.intersectsTriangle(v1, v2, v3);
                
                            // Step 5: Adjust the camera's height if there is an intersection
