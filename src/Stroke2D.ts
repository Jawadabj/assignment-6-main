...
public createSkyStrokeMesh(camera: gfx.Camera, skyRadius: number): gfx.Mesh3
    {
        ...
        const projectedMesh = new gfx.Mesh3();
        projectedMesh.material = new gfx.UnlitMaterial();
        projectedMesh.material.setColor(this.color); 

        const projectedVertices: gfx.Vector3[] = [];
        const ray = new gfx.Ray3();

        const boundingSphere = new gfx.BoundingSphere(); 
        boundingSphere.radius = skyRadius;
        this.vertices.forEach(vertex => {
            ray.setPickRay(vertex, camera);
            const intersection = ray.intersectsSphere(boundingSphere);

            if (intersection) {
                    projectedVertices.push(intersection);
            }
        });
        
        projectedMesh.setVertices(projectedVertices);
        projectedMesh.setIndices(this.indices);
    ...
    public createBillboard(camera: gfx.Camera, anchorPointWorld: gfx.Vector3): Billboard
    {
        ...
        // newMesh.material = new gfx.UnlitMaterial();
        // newMesh.material.setColor(stroke2D.color);
        // Step 1: Get camera position in world coordinates
        ...
        const projectedVertices: gfx.Vector3[] = [];
        const ray = new gfx.Ray3();
        const plane = new gfx.Plane3();
        plane.point = anchorPointWorld;

        const normal = gfx.Vector3.subtract(camera.position, anchorPointWorld);
        normal.y = 0; 
        normal.normalize(); 
        plane.normal = normal;
        ...
        billboardMesh.setVertices(projectedVertices);
