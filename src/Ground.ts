 ...
 * Assignment originally developed by Daniel Keefe, implemented for GopherGFX by Evan Suma Rosenberg and revised by Isayas Berhe Adhanom
...
export class Ground extends gfx.Mesh3
{
    ...
    const planeNormal = new gfx.Vector3(
        groundEndPoint.x - groundStartPoint.x,
        0,
        groundEndPoint.z - groundStartPoint.z
    );
    planeNormal.normalize();

    const projectionPlane = new gfx.Plane3(groundStartPoint, planeNormal);

    // Step 2: Project the user's stroke onto the projection plane
    const silhouetteCurve: gfx.Vector3[] = stroke2D.getVertices3D().map((strokePoint: { x: number; y: number; z: number; }) => {
        const toStrokePoint = new gfx.Vector3(
            strokePoint.x - groundStartPoint.x,
            strokePoint.y - groundStartPoint.y,
            strokePoint.z - groundStartPoint.z
        );
        const distanceToPlane = toStrokePoint.dot(planeNormal);
        return new gfx.Vector3(
            strokePoint.x - distanceToPlane * planeNormal.x,
            strokePoint.y - distanceToPlane * planeNormal.y,
            strokePoint.z - distanceToPlane * planeNormal.z
        );
    });

    // Step 3: Modify the ground vertices based on the projected stroke
    const influenceRadius = 10; // Define a radius of influence for the stroke

    for (let i = 0; i < this.vertices.length; i++) {
        const vertex = this.vertices[i];
        const toVertexStart = new gfx.Vector3(
            vertex.x - groundStartPoint.x,
            vertex.y - groundStartPoint.y,
            vertex.z - groundStartPoint.z
        );
        const distanceToPlane = toVertexStart.dot(planeNormal);
        const closestPointOnPlane = new gfx.Vector3(
            vertex.x - distanceToPlane * planeNormal.x,
            vertex.y - distanceToPlane * planeNormal.y,
            vertex.z - distanceToPlane * planeNormal.z
        );

        // Find the minimum distance to the silhouette curve
        let minDistance = Infinity;
        for (const curvePoint of silhouetteCurve) {
            const dx = closestPointOnPlane.x - curvePoint.x;
            const dz = closestPointOnPlane.z - curvePoint.z; // Ignore y for 2D distance
            const distance = Math.sqrt(dx * dx + dz * dz);
            minDistance = Math.min(minDistance, distance);
        }

        // Modify vertex height if within influence radius
        if (minDistance < influenceRadius) {
            const weight = Math.max(0, 1 - (minDistance / influenceRadius) ** 2);
            const hValue = this.computeH(closestPointOnPlane, silhouetteCurve, projectionPlane);
            vertex.y += weight * hValue; // Adjust vertex height with weighted contribution
        }
    }
